...
import { skapaKursMedAi } from "@/lib/kurs.functions";
...
const stegLabels = ["Laddar upp...", "AI lyssnar...", "AI skriver kurs...", "Klar!"];

function LaddaUpp() {
  const { user, profil } = useAuth();
  const navigate = useNavigate();
  const skapa = useServerFn(skapaKursMedAi);
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setSteg(1);
      const promise = skapa({ data: { kursnamn: namn, storage_path: path } });
  ...
  // referera navigate så lint inte klagar
  void navigate;
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-2xl space-y-6 px-6 py-10">
        <h1 className="text-3xl font-bold text-primary">Ladda upp video</h1>

        {!klar && steg === -1 && (
          <form onSubmit={onSubmit} className="card-shadow space-y-4 rounded-2xl bg-card p-6">
            <div>
            ...
            <div>
              ...
              <Input type="file" accept="video/*,audio/*" required onChange={(e) => setFil(e.target.files?.[0] ?? null)} />
            </div>
